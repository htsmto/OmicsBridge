...
A Conversion Table showing corresponding gene names between species will be generated, downloadable via "Download this table". If no ortholog exists for a gene, that entry remains blank. The "List of Converted Genes" on the right displays only genes with identified orthologs, ready to be copied and used in other sections. This tool utilises the Ensembl genome browser 113 (BioMart, GRCh38.p14 and GRCm39). Note that a single gene may have multiple orthologous matches.
...
A Conversion Table will be displayed in the middle, downloadable via "Download this table". Entries without matches remain blank. The List of Converted Genes on the right provides a clean list of successfully converted names. This tool utilises data from the Ensembl genome browser 113 (GRCh38.p14 and GRCm39).
...
### <u> **3.1. Find gene positions/annotations** </u>
...
The tool generates a table showing gene names (both Ensembl ID and gene symbol), chromosome, start position, end position, and strand information, downloadable via "Download this table". A copyable list of genes or coordinates appears on the right, ready for use in other sections—particularly useful when exploring ChIPseq, ATACseq, or using the Genome browser session. The conversion uses annotation files from [GENCODE](https://www.gencodegenes.org/human/release_42.html) (GRCh38.p13)
...
### <u> **3.2. Peak annotation** </u>

This tool annotates the genomic location of each input peak (e.g. a ChIPseq or ATACseq peak) and finds its nearest gene.

1. Enter the peak regions, one per line, in the format `chr:start-end` (e.g. `chr1:100000-100100`). The width of a peak must not exceed 100,000 bp.
2. Choose the genome (currently hg38 only) and click "Annotate the genomic locations".

If a peak cannot be annotated (e.g. wrong coordinate format, a chromosome not in the genome, or a peak wider than 100,000 bp), it is listed by name in a status message rather than silently dropped. Results for the successfully annotated peaks appear in three tabs:

=== "**Table**"
    A table of every annotated peak (labelled by its PeakID, `chr:start-end`) with its genomic feature annotation (e.g. Promoter, Exon, Intron, Downstream, Distal Intergenic) and nearest gene, downloadable via "Download this table".

=== "**Plots**"
    A pie chart (default) or horizontal stacked bar chart showing what proportion of the input peaks fall into each genomic feature category.

    ??? success "Adjustable graph parameters"
        - Figure size (width and height)
        - Legend title font size and legend text font size
        - X-axis font size (bar plot only)

=== "**Nearest gene names list**"
    A plain, copyable list of the nearest gene for each peak, shown either as Ensembl gene ID or gene symbol.
...
2. Choose a plot method:
    - "Calculate the percentile (stack bar plot)" — default
    - "Use the original count (stack bar plot)"
    - "Use the original count (dodge bar plot)"
...
??? success  "Adjustable graph parameters"
    ...
    - Font size (X label, Y label, title, and legend)
    - Colour for each of the two column groups
    - Use a white background
    - "Rotate X labels" switch (off by default), with an adjustable angle (default 45°) once enabled
