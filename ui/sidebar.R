sidebar <- dashboardSidebar(width = 300, collapsed = TRUE, 
    sidebarMenu(id='sidebar',
        ...
        menuItem(tagList("Count & Comparison", tags$br(), "Data Overview"), tabName='Data_Overview', icon=icon('chart-bar')),
        menuItem("Compare Across Datasets", tabName='Compare_across_datasets', icon=icon('chart-bar')),
        menuItem("Two Datasets Integration", tabName='Integrate_two_dataset', icon=icon('chart-bar')),
        menuItem("Clinical Data Analysis", tabName='Clinical_dataset', icon=icon('chart-bar')),
        menuItem("scRNA Data Overview", tabName='scRNA', icon=icon('chart-bar')),
        ...
        menuItem("Wiki (Documentation)", tabName='wiki_document', icon=icon('book'))
    ),
    tags$div(
        class = "sysmon-panel",
