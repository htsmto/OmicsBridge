tools_networkplot_UI <- function(ns){
    box(width=12, status='primary',  solidHeader = TRUE, title='Network plot',
        ...
        fluidRow(
            # Input
            column(4,
                box(width=12, title='Input the data',collapsible = TRUE, status='info',
                    fluidRow(
                        ...
                        column(12, materialSwitch(ns("Network_input_example"), 'Use an example data', value=FALSE,  status='info') )
                    ),
                ),
                box(width=12, title='Show the input data table',collapsible = TRUE, status='warning',
                    fluidRow(
                    ...
                    fluidRow(
                        ...
                        column(2, 
                            dropdownButton( h4(strong("Graph Settings")),
                                fluidRow(
                                column(6, selectInput(ns("Network_input_shape_from"), "The shape of node (From)", c('ellipse', 'circle', 'database', 'box', 'text', 'dot', 'star', 'triangle', 'triangleDown', 'square'), selected='ellipse')),
                                column(6, selectInput(ns("Network_input_shape_to"), "The shape of node (To)", c('ellipse', 'circle', 'database', 'box', 'text', 'dot', 'star', 'triangle', 'triangleDown', 'square'), selected='circle')),
                                column(6, colourpicker::colourInput(ns("Network_input_color_from"), "The colour of node (From)", value='#F7AFAF' )),
                                column(6, colourpicker::colourInput(ns("Network_input_color_to"), "The colour of node (To)", value='#B2E9FF' )),
