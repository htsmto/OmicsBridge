original_geneset_tableview_UI <- function(ns) {
  box(width=12, title=strong('Custom Gene Sets'), status='primary', collapsible = TRUE, solidHeader = TRUE,
    fluidRow(
      ...
      column(2, actionButton(ns('Original_geneset_delete_row'), 'Delete selected data', style="width:240px; color: #ffffff; background-color: #2d3cac; border-color: #1c48fa")),
      column(2, actionButton(ns('Original_geneset_Reload'), 'Reload data', style="width:240px; color: #ffffff; background-color: #2d3cac; border-color: #1c48fa")),
