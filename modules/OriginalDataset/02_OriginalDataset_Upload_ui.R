original_geneset_upload_UI <- function(ns) {
  box(width=12, title=strong('Add a Gene Set'), status='danger', collapsible = TRUE, solidHeader = TRUE,
    h3(''),
    fluidRow( 
      column(4, textInput(ns("Original_geneset_upload_Geneset_name"), "Geneset name *")), 
    ...
    fluidRow( 
      column(3, actionButton(ns('Original_geneset_upload_data'), 'Add the geneset to the list', style="color: #ffffff; background-color: #bc2929; border-color: #e130f9")), 
