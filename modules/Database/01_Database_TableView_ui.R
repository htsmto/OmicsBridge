database_tableview_UI <- function(ns){
  box(title='All the datasets registered in OmicsBridge', width=12, status='primary', solidHeader = TRUE,
    fluidRow(
      ...
      column(2, 
        h2('\n'),
        actionButton(ns('Reload'), 'Reload the database', style="width:200px; background-color: #2C43D1; color: white;"))
    ),
        ...
        fluidRow(
          column(6, 
            fluidRow(
              ...
              column(12, h5('')),
              column(6, actionButton(ns('reset_edit'), 'Reset edits', style="width:240px; background-color: #1328A8; color: white;"))
            )
          ),
